...
function Iris.WidgetConstructor(type: string, widgetClass: table)
	...
	if widgetClass.hasState then
		for _, v in Fields.IfState.Required do
			assert(
				widgetClass[v] ~= nil,
				`field {v} is missing from widget {type}, it is required for all widgets with state`
			)
			thisWidget[v] = widgetClass[v]
		end
	...
	if widgetClass.hasChildren then
		for _, v in Fields.IfChildren.Required do
			assert(
				widgetClass[v] ~= nil,
				`field {v} is missing from widget {type}, it is required for all widgets with children`
			)
			thisWidget[v] = widgetClass[v]
		end
...
function Iris._GenNewWidget(widgetType, arguments, widgetState, ID)
	...
	local widgetInstanceParent = if Iris._config.Parent
		then Iris._config.Parent
		else Iris._widgets[parentWidget.type].ChildAdded(parentWidget, thisWidget)
