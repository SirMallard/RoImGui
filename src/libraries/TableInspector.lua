...
local maxOpenedTableSize = Vector2.new(math.huge, math.huge)
...
function TableInspector.new()
	...
	self.backFrame.Parent = playerGui.RoImGui
	...
	self._connections[1] = game:GetService("RunService").RenderStepped:Connect(function()
		...
		for _, frame in self._hoveringFrames do
...
function TableInspector:drag(exitInputType)
	...
	self._dragConnection1 = game:GetService("RunService").RenderStepped:Connect(function()
		...
		local newScalePosition = (newPosition - self.basisFrame.Parent.AbsolutePosition)
			/ self.basisFrame.Parent.AbsoluteSize
	...
	self._dragConnection2 = game:GetService("UserInputService").InputEnded:Connect(function(inputObject)
		if inputObject.UserInputType ~= exitInputType then
			return
		end
...
function TableRoot:drag(exitInputType)
	...
	self._dragConnection1 = game:GetService("RunService").RenderStepped:Connect(function()
		...
		local newScalePosition = (newPosition - self.backFrame.Parent.AbsolutePosition)
			/ self.backFrame.Parent.AbsoluteSize
...
function Element:inputEnded(inputObject)
	...
	if not (controlPressed or shiftPressed) then
		local t = os.clock()
...
function Element:inputBegan(inputObject)
	...
	if controlPressed and not shiftPressed then -- pulls out the literal table
		if type(self._value) ~= "table" then
			return
		end
		if not self.parent or not self.parent.pullOutElement then
			return
		end
		...
		return true
	elseif shiftPressed and not controlPressed then -- pulls out the pathway
		-- if type(self._value) ~= "table" then return end
		if not self.parent or not self.parent.pullOutElement then
			return
		end
		if self.parent:getIndexElement() == self then
			return
		end
		-- pull this off and make a new root
		local name = tostring(self.parent:getIndex())
		...
		local t = os.clock()
		if
			t - self._lastExpansionAttempt < doubleClickPeriod
			and (self._lastExpansionAttemptPosition - getMousePosition()).magnitude < 4
		then
...
function Element:inputChanged(inputObject)
	if inputObject.UserInputType == Enum.UserInputType.MouseWheel then
		if not self._scrollingEnabled then
			return
		end
		local delta = inputObject.Position.z
		local lateralScroll = self._lateralScrollEnabled
...
function Element:remapEntries()
	...
	for _, entry in orphanedEntries do
	...
	for _, valueElement in orphanedValueElements do
...
function Element:renderTable()
	if not self._isExpanded then
		...
		local text = `{k}k {i}i`
		if self.dataFrame.Text == text then
			return
		end -- source of bugs

		local dataSize = TextService:GetTextSize(text, fontSize, font, Vector2.zero)
		local clipSize = Vector2.new(math.min(dataSize.x, maxClosedSize.x), math.min(dataSize.y, maxClosedSize.y))
	...
	local clipSize = Vector2.new(math.min(dataSize.x, maxOpenedTableSize.x), math.min(dataSize.y, maxOpenedTableSize.y))
	local backSize = clipSize + 2 * tableBorder * Vector2.one
...
function Element:setText(text)
	...
	if self._isExpanded then
		...
		clipSize = Vector2.new(math.min(dataSize.x, maxOpenedSize.x), math.min(dataSize.y, maxOpenedSize.y))

		self._scrollingEnabled = dataSize ~= clipSize
		self._lateralScrollEnabled = dataSize.y == clipSize.y
	else
		dataSize = TextService:GetTextSize(text, fontSize, font, Vector2.zero)
		clipSize = Vector2.new(math.min(dataSize.x, maxClosedSize.x), math.min(dataSize.y, maxClosedSize.y))
...
function Entry:update()
	...
	self.separatorFrame.Position =
		UDim2.fromOffset(padX + indexSize.x + padX, padY + math.min(indexSize.y, valueSize.y) / 2 - separatorSize.y / 2)
